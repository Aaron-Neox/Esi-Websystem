...
export default class Mail extends React.Component {
    constructor(props){
		super(props);
		this.state = {
			teacherList : this.getlist()
		}
	}
	getlist(){
		axios 
		.get('http://localhost:8000/api/teacher/list')
		.then(res=>{
			this.setState({
	...
	RemoveMethod(id,index){
		$.post('http://localhost:8000/api/teacher/destroy',{
			uiddelet:id
		})
		.then(res=>{
			console.log(res)
			var list=this.state.teacherList
			list.splice(index , 1);
			this.setState({
				teacherList : list
			})
	...
	AddMethod(){
		...
		$.post('http://localhost:8000/api/teacher/add',{
			nom:$('#name').val(),
			prenom:$('#lastn').val(),
			sid:$('#uid').val(),
			degree:$('#deg').val()
        ...
        return(
				<div className="Two-container">
					...
					<div className="title">
						...
						<ol>
							{Array.isArray(this.state.teacherList) ? this.state.teacherList.map(item => {
								return(
									...
									<ul className="list-mini-item">
										...
										type="button" 
										onClick={() => this.RemoveMethod(item.id,this.state.teacherList.indexOf(item)) }>	
											&minus;
										</button>
									</div>
									<hr/>
